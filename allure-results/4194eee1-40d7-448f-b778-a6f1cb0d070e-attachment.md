...
```ts
  ...
  16 |     
  17 |     // expect(errorMessage).toContain('Invalid credentials')
  18 | 
  19 |     expect(errorMessage?.includes('Invalid credential')).toBeTruthy()
  20 | 
> 21 |     expect(errorMessage==='Invalid credential').toBeTruthy()
     |                                                 ^ Error: expect(received).toBeTruthy()
  22 | 
  23 | });
```