...
Call log:
  - waiting for locator('//p[contains(@text(),"Invalid credentials")]')
...
```ts
  ...
  10 | 
  11 |     const errorMessage = page.locator('//p[contains(@text(),"Invalid credentials")]')
  12 | 
> 13 |     console.log('Error message is : ' + await errorMessage.textContent())
     |                                                            ^ Error: locator.textContent: Test timeout of 30000ms exceeded.
  14 |     console.error(errorMessage)
  15 | 
  16 | });
```